const API_BASE = `${window.location.origin}/api/challenge3`;

let timerId = null;
let elapsedSeconds = 0;
let solved = false;
let promptValue = "student@ctf-lab:~$";
...
function setPrompt(prompt) {
  promptValue = prompt || "student@ctf-lab:~$";
...
function applyChallengeData(challenge) {
  ...
  document.getElementById("challengeTitle").textContent = challenge.title || "The Lost Flag";
  document.getElementById("difficultyBadge").textContent = difficulty;
  applyDifficultyBadgeStyle(difficulty);
  document.getElementById("categoryValue").textContent = challenge.category || "Linux";
...
function applySessionState(sessionState) {
  ...
  setPrompt(sessionState.terminal?.prompt || "student@ctf-lab:~$");

  const hintButton = document.getElementById("hintBtn");
  const submitButton = document.getElementById("submitBtn");
  const flagInput = document.getElementById("flag");

  hintButton.disabled = solved || Number(sessionState.hintsRemaining || 0) <= 0;
  submitButton.disabled = false;
  flagInput.disabled = solved;
...
function moveHistoryCursor(direction) {
  if (localCommandHistory.length === 0) {
    return;
  }
...
document.addEventListener("DOMContentLoaded", async () => {
  setupEvents();
  updateTimerDisplay();
  appendTerminalLine("Welcome to the Linux training terminal.");
  appendTerminalLine("Type 'help' to view available commands.");
  await loadChallenge();
});
