...
const { getOrCreateProgress, setProgress, getProgress } = require("../lib/progressStore");
...
const { challengeId, challengesDbPath, completionLogPath } = require("../config/challenge2Config");

const dynamicChallengeId = "challenge2";
const dynamicChallengeSlug = "challenge2";
const hiddenAdminBackupPath = path.resolve(__dirname, "../targets/challenge2-app/internal/admin-backup.html");
const legacyStaticFlag = "CTF{admin_panel_exposed}";
...
function buildAdminBackupPage(flagValue) {
  const templateHtml = fs.readFileSync(hiddenAdminBackupPath, "utf8");
  return templateHtml.replace(legacyStaticFlag, escapeHtml(flagValue));
}
...
function appendCompletionLog(entry) {
  try {
    ...
    console.error("Failed to write challenge2 completion log:", error.message);
  }
}

function serveInternalAdminBackup(req, res) {
  try {
    if (!isDynamicFlagsEnabled()) {
      return res.sendFile(hiddenAdminBackupPath);
    }

    const user = getSessionUser(req);
    const existingState = user ? getProgress(user.id, challengeId) : null;
    const dynamicContext = resolveDynamicContext(req, user, existingState, {
      createIfMissing: !(existingState && existingState.solved)
    });
    ...
    const placeholderToken = "SESSION_TOKEN_UNAVAILABLE";
    const token = dynamicContext.expectedFlag || placeholderToken;
    const html = buildAdminBackupPage(token);
    return res.type("html").send(html);
  } catch (error) {
    return res.status(500).send("Unable to load internal backup console.");
  }
}
...
function getChallenge(req, res) {
  try {
    ...
    if (state.solved && challenge.explanation) {
      responseBody.explanation = challenge.explanation;
    }
  ...
  try {
    ...
    if (state.solved) {
      ...
      if (challenge.explanation) {
        alreadySolved.explanation = challenge.explanation;
      }
    ...
    if (isDynamicFlagsEnabled()) {
      ...
      if (dynamicContext.errorCode === "NO_ACTIVE_INSTANCE") {
        return res.status(409).json({
          success: false,
          message: "No active challenge instance for this session. Reopen Challenge 2 and try again."
        });
      }
    ...
    if (isCorrect) {
      ...
      if (activeDynamicInstance && activeDynamicInstance.instanceId) {
        try {
          ...
          console.error("Failed to mark challenge2 instance solved:", error.message);
        }
      }
      ...
      if (challenge.explanation) {
        successResponse.explanation = challenge.explanation;
      }
