...
const { challengeId, challengesDbPath, completionLogPath, commandMaxLength } = require("../config/challenge3Config");
const {
  createTerminalState,
  executeTerminalCommand,
  formatPromptPath
} = require("../lib/challenge3Terminal");

const dynamicChallengeId = "challenge3";
const dynamicChallengeSlug = "challenge3";
...
function buildSessionSnapshot(req, challenge, state) {
  ...
  return {
    ...
    terminal: {
      ...
      prompt: `student@ctf-lab:${formatPromptPath(state.terminal.cwd)}$`,
...
function appendCompletionLog(entry) {
  try {
    ...
    console.error("Failed to write challenge3 completion log:", error.message);
  }
}
    ...
    if (isDynamicFlagsEnabled()) {
      ...
      if (dynamicContext.errorCode === "NO_ACTIVE_INSTANCE") {
        return res.status(409).json({
          success: false,
          message: "No active challenge instance for this session. Reopen Challenge 3 and try again."
        });
      }
    ...
    if (isCorrect) {
      ...
      if (activeDynamicInstance && activeDynamicInstance.instanceId) {
        try {
          ...
          console.error("Failed to mark challenge3 instance solved:", error.message);
        }
      }
